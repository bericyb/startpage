...
let insertionSort = (inputArr) => {
    ...
    for (let i = 1; i < length; i++) {
        let key = inputArr[i].priority;
        let j = i - 1;
        while (j >= 0 && inputArr[j] > key) {
            inputArr[j + 1].priority = inputArr[j].priority;
            j = j - 1;
        }
        inputArr[j + 1].priority = key;
    }
    return inputArr;
};
...
function displayTasks() {
    for (var i = 0; i < taskList.length; i++) {
        document.getElementById('list').innerHTML = (i+1 +". " + taskList[i].name + " " + taskList[i].date);
    }
}

function addTask() {
    task.name = document.getElementById('taskField').value;
